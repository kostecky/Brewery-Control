...
int CYCLE_RESOLUTION = 50; // Check the potentiometer every 50ms to see if we need to adjust the duty cycle
...
void loop() {
  ...
  if ((cycle_time_left <= 0) || (pot_val != last_pot_val)) {
    ...
    if (cycle_time_left <= time_to_switch_state) {
      digitalWrite(SSR_PIN, LOW);
    }
